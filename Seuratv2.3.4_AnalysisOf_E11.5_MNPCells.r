############ This script processes E11.5 MNP Cells using Seurat [v2.3.4]###############
...
M <- CreateSeuratObject(raw.data = data, min.cells =3, min.genes = 200, project= 'E11.5_MNP')
######################################################################
...
dev.off()
M <- FilterCells(object = M, subset.names = c("nGene", "percent.mito"), low.thresholds = c(-Inf, -Inf), high.thresholds = c(5000, 0.08))
...
######################### Save Robject ###############################################################
save(M, file = 'Seurat_v2.3.4_AnalysisOf_E11.5_MNPCells.Robj')
######################################################################################################